...
class TestCloudRedisClient(object):
    ...
    def test_create_instance(self):
        ...
        expected_request = cloud_redis_pb2.CreateInstanceRequest(parent=parent, instance_id=instance_id, instance=instance)
    ...
    def test_update_instance(self):
        ...
        authorized_network = 'authorizedNetwork-1733809270'
        persistence_iam_identity = 'persistenceIamIdentity1061944584'
        expected_response = {'name': name_2, 'display_name': display_name_2, 'location_id': location_id, 'alternative_location_id': alternative_location_id, 'redis_version': redis_version, 'reserved_ip_range': reserved_ip_range, 'host': host, 'port': port, 'current_location_id': current_location_id, 'status_message': status_message, 'memory_size_gb': memory_size_gb_2, 'authorized_network': authorized_network, 'persistence_iam_identity': persistence_iam_identity}
        expected_response = cloud_redis_pb2.Instance(**expected_response)
        ...
        assert len(channel.requests) == 1
        expected_request = cloud_redis_pb2.UpdateInstanceRequest(update_mask=update_mask, instance=instance)
        actual_request = channel.requests[0][1]
        assert expected_request == actual_request
    ...
    def test_import_instance(self):
        ...
        authorized_network = 'authorizedNetwork-1733809270'
        persistence_iam_identity = 'persistenceIamIdentity1061944584'
        expected_response = {'name': name_2, 'display_name': display_name, 'location_id': location_id, 'alternative_location_id': alternative_location_id, 'redis_version': redis_version, 'reserved_ip_range': reserved_ip_range, 'host': host, 'port': port, 'current_location_id': current_location_id, 'status_message': status_message, 'memory_size_gb': memory_size_gb, 'authorized_network': authorized_network, 'persistence_iam_identity': persistence_iam_identity}
        expected_response = cloud_redis_pb2.Instance(**expected_response)
        operation = operations_pb2.Operation(name='operations/test_import_instance', done=True)
        operation.response.Pack(expected_response)
        ...
        assert len(channel.requests) == 1
        expected_request = cloud_redis_pb2.ImportInstanceRequest(name=name, input_config=input_config)
        actual_request = channel.requests[0][1]
        assert expected_request == actual_request
    ...
    def test_import_instance_exception(self):
        # Setup Response
        error = status_pb2.Status()
        operation = operations_pb2.Operation(name='operations/test_import_instance_exception', done=True)
        operation.error.CopyFrom(error)
    ...
    def test_export_instance(self):
        ...
        expected_response = {'name': name_2, 'display_name': display_name, 'location_id': location_id, 'alternative_location_id': alternative_location_id, 'redis_version': redis_version, 'reserved_ip_range': reserved_ip_range, 'host': host, 'port': port, 'current_location_id': current_location_id, 'status_message': status_message, 'memory_size_gb': memory_size_gb, 'authorized_network': authorized_network, 'persistence_iam_identity': persistence_iam_identity}
        expected_response = cloud_redis_pb2.Instance(**expected_response)
        operation = operations_pb2.Operation(name='operations/test_export_instance', done=True)
        operation.response.Pack(expected_response)
        ...
        assert len(channel.requests) == 1
        expected_request = cloud_redis_pb2.ExportInstanceRequest(name=name, output_config=output_config)
        actual_request = channel.requests[0][1]
        assert expected_request == actual_request
    ...
    def test_export_instance_exception(self):
        # Setup Response
        error = status_pb2.Status()
        operation = operations_pb2.Operation(name='operations/test_export_instance_exception', done=True)
        operation.error.CopyFrom(error)
    ...
    def test_failover_instance(self):
        ...
        expected_response = {'name': name_2, 'display_name': display_name, 'location_id': location_id, 'alternative_location_id': alternative_location_id, 'redis_version': redis_version, 'reserved_ip_range': reserved_ip_range, 'host': host, 'port': port, 'current_location_id': current_location_id, 'status_message': status_message, 'memory_size_gb': memory_size_gb, 'authorized_network': authorized_network, 'persistence_iam_identity': persistence_iam_identity}
        expected_response = cloud_redis_pb2.Instance(**expected_response)
        operation = operations_pb2.Operation(name='operations/test_failover_instance', done=True)
        operation.response.Pack(expected_response)
    ...
    def test_failover_instance_exception(self):
        # Setup Response
        error = status_pb2.Status()
        operation = operations_pb2.Operation(name='operations/test_failover_instance_exception', done=True)
        operation.error.CopyFrom(error)
    ...
    def test_delete_instance(self):
        ...
        operation = operations_pb2.Operation(name='operations/test_delete_instance', done=True)
        operation.response.Pack(expected_response)
    ...
    def test_delete_instance_exception(self):
        # Setup Response
        error = status_pb2.Status()
        operation = operations_pb2.Operation(name='operations/test_delete_instance_exception', done=True)
        operation.error.CopyFrom(error)
    ...
    def test_upgrade_instance(self):
        ...
        authorized_network = 'authorizedNetwork-1733809270'
        persistence_iam_identity = 'persistenceIamIdentity1061944584'
        expected_response = {'name': name_2, 'display_name': display_name, 'location_id': location_id, 'alternative_location_id': alternative_location_id, 'redis_version': redis_version_2, 'reserved_ip_range': reserved_ip_range, 'host': host, 'port': port, 'current_location_id': current_location_id, 'status_message': status_message, 'memory_size_gb': memory_size_gb, 'authorized_network': authorized_network, 'persistence_iam_identity': persistence_iam_identity}
        expected_response = cloud_redis_pb2.Instance(**expected_response)
        operation = operations_pb2.Operation(name='operations/test_upgrade_instance', done=True)
        operation.response.Pack(expected_response)
        ...
        assert len(channel.requests) == 1
        expected_request = cloud_redis_pb2.UpgradeInstanceRequest(name=name, redis_version=redis_version)
        actual_request = channel.requests[0][1]
        assert expected_request == actual_request
    ...
    def test_upgrade_instance_exception(self):
        # Setup Response
        error = status_pb2.Status()
        operation = operations_pb2.Operation(name='operations/test_upgrade_instance_exception', done=True)
        operation.error.CopyFrom(error)
